  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        ...
        assigneeNames: {
          type: Type.ARRAY,
          description: `למי מיועדת המשימה. השתמש בשמות מהרשימה: [${userNames.join(', ')}]. אם נאמר "שנינו" או "כולנו", השתמש ב-['all']. ברירת המחדל היא ['${userNames[0]}'].`,
    ...
    return {
      ...
      parameters: {
        ...
        properties: {
          ...
          assigneeNames: {
            type: Type.ARRAY,
            description: `רשימת האנשים החדשה שהמשימה מיועדת להם מהרשימה: [${userNames.join(', ')}, all].`,
            items: { type: Type.STRING },
          },
    ...
    if (functionCalls && functionCalls.length > 0) {
      ...
      if (name === 'createTask' || name === 'updateTask') {
        if (args.assigneeNames && Array.isArray(args.assigneeNames)) {
          const assigneeIds: string[] = args.assigneeNames.map((assigneeName: string) => {
          ...
          // If 'all' is present, it should be the only one.
          if (assigneeIds.includes('all')) {
            args.assigneeIds = ['all'];
          } else if (assigneeIds.length > 0) {
            args.assigneeIds = [...new Set(assigneeIds)]; // Ensure unique IDs
          }
        }
        delete args.assigneeNames;
        return args;
      }
    }
...
export const parseTaskUpdateFromText = async (text: string, users: User[], existingTask: Task): Promise<any> => {
    ...
    const systemInstruction = `
        ...
        - מיועד ל: ${existingTask.assigneeIds?.map(id => {
            if (id === 'all') return 'כולם';
            return users.find(u => u.id === id)?.name;
        }).filter(Boolean).join(', ') || 'לא הוקצה'}
