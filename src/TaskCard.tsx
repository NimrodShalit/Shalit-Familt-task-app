...
const getAssigneeDetails = (assigneeIds: string[], users: User[]) => {
    ...
    if (assigneeNames.length === 1) {
        ...
        if (user) {
            ...
            return { name: assigneeNames[0], color: colors[colorIndex] };
        }
    }

    return { name: assigneeNames.join(', '), color: 'bg-gray-200 text-gray-800' };
}
...
export const TaskCard: React.FC<TaskCardProps> = ({ task, users, onToggleComplete, onEdit, onDelete }) => {
    ...
    const assigneeDetails = getAssigneeDetails(task.assigneeIds, users);
